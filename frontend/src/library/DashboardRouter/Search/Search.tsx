  ...
  return (
    <div className="w-full flex justify-center content-center">
      <div className="m-5 sm:p-8 p-4 rounded-lg Shadow BOBorder border-2 w-[95%] h-min">
        <h1 className="sm:text-4xl text-2xl font-bold mb-4 text-black text-center italic">Search</h1>
        ...
        <div className="bg-white bg-opacity-70 px-2 py-6 BOBorder border-y-2">
          ...
          <div className="flex justify-center mb-4">
            <label className="inline-flex items-center mx-2">
              ...
              />
              <span className="ml-2 text-black sm:text-base text-xs">Albums</span>
            </label>
            ...
            <label className="inline-flex items-center mx-2">
              <input
                ...
                className="form-checkbox h-[17px] w-[17px] text-black font-bold"
              />
              <span className="ml-2 text-black sm:text-base text-xs">Artists</span>
            </label>
            ...
            <label className="inline-flex items-center mx-2">
              <input
                ...
                className="form-checkbox h-[17px] w-[17px] text-black font-bold"
              />
              <span className="ml-2 text-black sm:text-base text-xs">Tracks</span>
            </label>
          </div>
          ...
          <div className=" flex ">
            <input
              ...
              placeholder="Search"
              className="w-full sm:h-10 h-8 px-3 sm:text-base text-sm placeholder-black border-2 border-gray-600 rounded-l-xl drop-shadow-md
            ...
            <button
              ...
              hover:border- focus:outline-none hover:ring-2 focus:border-black hover:ring-opacity-50 sm:text-base text-sm"
            >
              Search
            </button>
          </div>
        ...
        {/*  Results section  */}
        <div className="mt-4">
          <h1 className="sm:text-2xl text-xl font-bold mb-4 text-black text-center italic">Results</h1>
          <div className="flex justify-center">
            <div ref={resultsDiv} className="w-[95%] h-[500px] overflow-scroll bg-white bg-opacity-70 BOBorder border-2 rounded-lg">
              <div className="m-5">
                {!results &&
                  <h1 className="sm:text-2xl text-base font-bold text-black text-center italic">
                ...
                <div className="flex flex-col justify-between w-[50%]">
                  <div>
                    <HashLink
                      ...
                      className="lg:text-lg text-sm font-bold hover:cursor-pointer"
                    >
                      {album.name}
                    </HashLink>
                    <p className="sm:text-sm text-xs font-bold text-gray-700">{album.artists.map((artist) => artist.name).join(', ')}</p>
                    <p className="sm:text-sm text-xs text-gray-500">{album.release_date.slice(0, 4)}</p>
                  </div>
                  <p className="sm:text-sm text-xs text-gray-700 font-bold mt-2">album</p>
                </div>
                <HashLink to={`/dashboard/pages/album?id=${album.id}#top`}>
                  <img
                    src={album.images[0]?.url || noImageURL} alt={album.name}
                    className="lg:h-32 h-20 lg:w-32 w-20 rounded-xl hover:cursor-pointer border-black border"
                  />
                </HashLink>
              </div>
            )
              ...
              >
                <div className="flex flex-col justify-between w-[50%]">
                  <div>
                    ...
                    <p className="sm:text-sm text-xs text-gray-500">{artist.genres.join(', ')}</p>
                  </div>
                  <p className="sm:text-sm text-xs text-gray-700 font-bold mt-2">artist</p>
                </div>
              ...
              >
                <div className="flex flex-col justify-between w-[50%]">
                  <div>
                    ...
                    <p className="sm:text-sm text-xs font-bold text-gray-700">{track.artists.map((artist) => artist.name).join(', ')}</p>
                    <p className="sm:text-sm text-xs font-bold text-gray-500">{track.album.name}</p>
                    <p className="sm:text-sm text-xs text-gray-500">{msToMinutesSeconds(track.duration_ms)}</p>
                  </div>
                  <p className="sm:text-sm text-xs text-gray-700 font-bold mt-2">track</p>
                </div>
